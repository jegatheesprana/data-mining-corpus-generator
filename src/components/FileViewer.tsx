...
const FileViewer = ({data, setData, onExport, handleSaveLocal, handleReset}: any) => {
    ...
    const handleAddMetaphor = () => {
        if (metaphors.length<5) {
            setMetaphors(metaphors=>([...metaphors, {metaphor: '', meaning: ''}]))
        } else {
    ...
    const handleCreate = () => {
        if (!metaphors[metaphors.length-1].metaphor && !metaphors[metaphors.length-1].meaning) {
            setMetaphors((metaphors=>{
                const newMeta = [...metaphors]
                newMeta[newMeta.length-1] = {metaphor: selection, meaning: ''}
                return newMeta
            }))
        } else {
            if (metaphors.length<5) {
                setMetaphors((metaphors=>{
                    const newMeta = [...metaphors]
                    newMeta[newMeta.length] = {metaphor: selection, meaning: ''}
                    return newMeta
                }))
            } else {
                setMessage("Max metaphor is 5")
    ...
    useEffect(()=>{
        ...
        for (let i = 1; i < 6; i++) {
            if (data[currentSong][`metaphor_${i}`]) {
                metaphors.push({metaphor: data[currentSong][`metaphor_${i}`], meaning: data[currentSong][`metaphor_${i}_meaning`]})
            }
        }
        if (metaphors.length) {
            setMetaphors(metaphors)
    ...
    return (
        ...
        <Box sx={{ flexGrow: 1, mt: 2 }}>
            <Grid container spacing={2}>
                ...
                {/* @ts-ignore: Unreachable code error */}
                    <HtmlTooltip
                        title={
                        <>
                            {columns.filter((col: any)=>col.edit).map((column: any)=><Typography color="inherit" key={column.field}>{`${column.name}: ${data[currentSong][column.field]}`}</Typography>)}
                        </>
                        }
                    >
                        <div>
            ...
            <Grid container spacing={2}>
                ...
                <Grid item >
                    <Button variant="contained" fullWidth onClick={handleCreate} disabled={!selection}>Create</Button>
                </Grid>
                <Grid item >
                ...
                <Grid item xs={6} sx={{height: 'calc(100vh - 150px)', overflow: 'auto'}} ref={metaphorRef}>
                    <Stack spacing={2}>
                        {metaphors.map((metaphor:any, id: any)=>(
                            <Card key={id}>
                                <CardHeader
                                    action={
                                    <IconButton aria-label="settings" onClick={()=>handleRemoveMetaphor(id)}>
                                        <DeleteIcon />
                                    </IconButton>
                                    }
                                ...
                                <CardContent sx={{pt: 0}}>
                                    <TextField
                                        ...
                                        onChange={(e)=>handleMetaphorChange(id, 'metaphor', e)}
                                        fullWidth
                                        sx={{mb: 1}}
                                    />
                                    <TextField
                                        ...
                                        onChange={(e)=>handleMetaphorChange(id, 'meaning', e)}
                                        fullWidth
                                    />
                                </CardContent>
                            </Card>
                        ))}
                        <Button variant="outlined" onClick={handleAddMetaphor} >Add</Button>
                    </Stack>
                    {/* <TextField
                        id="outlined-multiline-static"
                        label="Metaphor"
